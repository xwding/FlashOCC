...
# POINT_CLOUD_RANGE = [-120, -40, -1, 120, 40, 9]
# POINT_CLOUD_RANGE = [0, -20, -2, 120, 20, 4]
...
def voxel2points(voxel, occ_show, voxelSize):
    ...
    # FIX: 添加半个体素尺寸偏移量以获得真正的中心位置
    points = torch.cat((occIdx[0][:, None] * voxelSize[0] + voxelSize[0] / 2 + POINT_CLOUD_RANGE[0], \
                        occIdx[1][:, None] * voxelSize[1] + voxelSize[1] / 2 + POINT_CLOUD_RANGE[1], \
                        occIdx[2][:, None] * voxelSize[2] + voxelSize[2] / 2 + POINT_CLOUD_RANGE[2]),
                       dim=1)      # (N, 3) 3: (x, y, z)
...
def voxel_profile(voxel, voxel_size):
    """
    Args:
        voxel: (N, 3)  3:(x, y, z) [已经修正为体素中心的坐标]
        voxel_size: (vx, vy, vz)

    Returns:
        box: (N, 7) (x, y, z, vx, vy, vz, 0) [直接使用中心坐标]
    """
    centers = voxel  # 直接使用体素中心坐标
...
def show_point_cloud(points: np.ndarray, colors=True, points_colors=None, bbox3d=None, voxelize=False,
    ...
    if voxelize:
        ...
        line_sets.points = o3d.open3d.utility.Vector3dVector(bbox_corners.reshape((-1, 3))+offset - np.array([0,0,voxel_size/2]).reshape((-1, 3)))
        line_sets.lines = o3d.open3d.utility.Vector2iVector(linesets.reshape((-1, 2)))
        line_sets.paint_uniform_color((0, 1, 0))
...
def generate_the_ego_car():
    ...
    ego_point_x = (ego_xyz[:, 0:1] + 0.5) / ego_xdim * (ego_range[3] - ego_range[0]) + ego_range[0] +1.6 
    ego_point_y = (ego_xyz[:, 1:2] + 0.5) / ego_ydim * (ego_range[4] - ego_range[1]) + ego_range[1] +2
...
def parse_args():
    ...
    parser.add_argument(
        '--root_path',
        type=str,
        default='/media/dxw/bacc88ab-8909-4a30-a90c-1815997d26163/dxw/occ/nuscenes/nuscenes/train',
        help='Path to nuScenes dataset')
    parser.add_argument(
        '--save_path',
        type=str,
        default='/home/dxw/Desktop/jobcode/mos1.2/app/occupancy/test',
...
def main():
    ...
    info_path= "/media/dxw/bacc88ab-8909-4a30-a90c-1815997d26163/dxw/occ/nuscenes/nuscenes/train/nuscenes_pkl/nuscenes_infos_train.pkl"
    dataset = pickle.load(open(info_path, 'rb'))
    # prepare save path and medium
    vis_dir = args.save_path
    vis_dir = "/media/dxw/bacc88ab-8909-4a30-a90c-1815997d26163/dxw/occ/data/occ_3d/dxw_show"
    ...
    config_path="/home/dxw/Desktop/jobcode/mos1.2/app/occupancy/SurroundOcc/tools/generate_occupancy_nuscenes_maxieye_own/config.yaml"
    import yaml
    ...
    occ_3d ="/media/dxw/bacc88ab-8909-4a30-a90c-1815997d26163/dxw/occ/data/occ_3d/gts/"
    nusc = NuScenes(version="v1.0-trainval", dataroot="/media/dxw/bacc88ab-8909-4a30-a90c-1815997d26163/dxw/occ/data/nuscenes", verbose=True)
    ...
    def key_callback(vis):
        ...
        # 辅助函数：遍历某个 scene 的所有 sample
    def iter_scene_samples(nusc_, scene_rec):
        sample_token_ = scene_rec['first_sample_token']
        while sample_token_:
            sample_ = nusc_.get('sample', sample_token_)
            yield sample_
            sample_token_ = sample_['next']
    vis = o3d.visualization.VisualizerWithKeyCallback()
    vis.create_window()
    vis.register_key_callback(32, key_callback)  # 注册空格键回调
    # 打印场景索引和名称
    global pause_flag
    for idx, scene in enumerate(scenes):
        print(f"场景索引: {idx}, Token: {scene['token']}, 描述: {scene['description']}, 名称: {scene['name']}")
        # 获取场景中的所有样本
        for s in iter_scene_samples(nusc, scene):
            print(f"  样本 Token: {s['token']}")
            labels= occ_3d + scene['name'] + '/' + s['token'] + '/labels.npz'
            gt_data = np.load(labels)
            semantics_gt, mask_lidar, mask_camera = gt_data['semantics'], gt_data['mask_lidar'], gt_data['mask_camera']
            ...
            image_canvas=np.zeros((900 * 2, 1600 * 3, 3), dtype=np.uint8)
            image_canvas[:900, :, :] = np.concatenate(imgs[:3], axis=1)
            image_canvas[900:, :, :] = img_back
            image_canvas=cv2.resize(image_canvas, (1600, 900))
            cv2.imshow('front', imgs[1])
            cv2.imshow('big_img', image_canvas)
            
            # for i, img in enumerate(imgs):
            #     cv2.imshow(f'{views[i]}', img)
            # 键盘处理：空格暂停 / ESC 退出
            key = cv2.waitKey(1) & 0xFF
            if key == 32:  # 空格
                pause_flag = not pause_flag
                print("暂停" if pause_flag else "继续")
            elif key == 27:  # ESC
                print("退出")
                if args.format == 'video':
                    vout.release()
            ...
            while pause_flag:
                ...
                time.sleep(0.01)
            if args.format == 'image':
                out_dir = os.path.join(vis_dir, f"{scene['name']}", f"{s['token']}")
                # mmcv.mkdir_or_exist(out_dir)
                ...
                cv2.putText(big_img, f"{idx:{idx}}", (5, 15), fontFace=cv2.FONT_HERSHEY_COMPLEX, color=(0, 0, 0),
                            fontScale=0.5)
                cv2.putText(big_img, f"{scene['name']}", (5, 35), fontFace=cv2.FONT_HERSHEY_COMPLEX, color=(0, 0, 0),
                            fontScale=0.5)
                cv2.putText(big_img, f"{s['token'][:5]}", (5, 55), fontFace=cv2.FONT_HERSHEY_COMPLEX, color=(0, 0, 0),
                            fontScale=0.5)
                vout.write(big_img)
